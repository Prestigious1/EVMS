...
def build_liability_receipt_pdf(
    *,
    ...
    if liability_type == "DAMAGE":
        total_paid = sum(p.amount for p in Payment.objects.filter(damage_report=liability, status=PaymentStatus.PAID))
    else:
        total_paid = sum(p.amount for p in Payment.objects.filter(penalty=liability, status=PaymentStatus.PAID))
        
    outstanding = total_amount - total_paid
    if outstanding < 0 or liability.is_paid:
